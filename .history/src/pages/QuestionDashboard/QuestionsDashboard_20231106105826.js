...
export const QuestionsDashboard = () => {
  ...
  const [questionNumber, setQuestionNumber] = useState(48);
  const getQuestionBg = (index) => {
    if (index > questionNumber) return "white";
    else if (index === questionNumber) return "blue";
    else if (index < questionNumber) {
      const lst = state.list[index];
      if (lst.selected == null) return "gray";
      else return "green";
    }
  };
  const setNewIndex = (id) => {
    if (id != null) {
      const ind = state.list.findIndex([state.list.id == id, id]);
    }
  };
  return (
    <>
